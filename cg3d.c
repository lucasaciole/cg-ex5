...
int RasterFill3D(object * face, bufferdevice * dev, viewport * port, int color) {
  int flag = 0;
  int a = 0;
  int b = 0;
  for (int i = 0; i < dev->MaxY; i++) {
    for (int j = 0; j < dev->MaxX; j++) {
      int i_curr_cell = i * dev->MaxX + j;      
      if (!flag) {
        if (dev->buffer[i_curr_cell] == color)  {
          a = j;
          flag = !flag;
        }
      } else {
        if (dev->buffer[i_curr_cell] == color) {
          ...
          // fill line between a and b;
          for (int k = a; k < b; k++) {
            dev->buffer[i * dev->MaxX + k] = color;
          }

          // Keep looking for possible 
          a = b;
          b = 0;
        }
      }

      // printf("%d", dev->buffer[i_curr_cell]);
    }
    // Reset the flag and positions so we won`t fill till the/from the border by mistake;
    a = 0;
    b = 0;
    flag = 0;
    // printf("\n");
