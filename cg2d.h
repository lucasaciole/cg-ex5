...
matrix * SetRotMatrix(float);  // retorna uma matriz para a operação de rotação
matrix * SetSclMatrix(float, float);   // retorna uma matriz para a operação de escalonamento
matrix * SetSftMatrix(float, float);   // retorna uma matriz para a operação de deslocamento
point * LinearTransf(matrix *, point *);  // realiza a transformação linear sobre um ponto
matrix * ComposeMatrix(matrix *, matrix *); // retorna uma matriz resultado da composição de outras duas
int RasterFill(bufferdevice * dev, viewport * port, int color);

/* Operações lineares sobre objetos no SRU 2D */
object * TransObj(object *, matrix *); // retorna um objeto resultado de uma operação linear
